...
class Visualizer:
    ...
    def visualize_inference_prediction(self, global_step: int=0):
    ...
    def _pred_route(self):
        ...
        if self.config.predict_ddbm:
            if self.config.ddbm_training:
                pred_route = self.predictions.pred_route['best_reg'][...,:-1,:]
            else:
                pred_route = self.predictions.pred_route[...,:-1,:]
        else:
...
def visualize_feature_maps(
    ...
    images = [
        ...
        (
            # BridgeDrive
            predictions.pred_route['best_reg'][...,:-1,:].to(dtype=torch.float32).detach().cpu().numpy()
            if config.predict_ddbm
            # else predictions.pred_route[0].detach().cpu().numpy(),
