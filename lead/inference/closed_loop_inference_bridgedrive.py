...
class ClosedLoopInference(OpenLoopInference):
    ...
    def execute_route_and_target_speed(
        ...
        if self.config_closed_loop.diffusion_speed:
            # NumPy has no bfloat16 dtype. Mixed-precision inference can return
            # BF16 predictions, so convert tensors to FP32 at the CPU/NumPy
            # boundary used by the controllers.
            tmp_pred_route = pred_checkpoints[0].detach().float().cpu().numpy()
            pred_checkpoints = tmp_pred_route[:10, :]
            speed = float(speed[0].item())
            pred_target_speed = float(tmp_pred_route[10,0])
            print("use diffusion speed")
        else:
            pred_checkpoints = (
                pred_checkpoints[0].detach().float().cpu().numpy()
            )
            speed = float(speed[0].item())
            pred_target_speed = float(pred_target_speed[0].item())
    ...
    def execute_waypoints(
        ...
        Returns:
            ...
            brake: Brake command in [0, 1]
        """
        waypoints = waypoints[0].detach().float().cpu().numpy()
        speed = float(velocity[0].item())
