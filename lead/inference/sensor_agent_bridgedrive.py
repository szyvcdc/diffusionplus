...
from lead.inference.closed_loop_inference_bridgedrive import (
    ClosedLoopInference,
...
from lead.visualization.visualizer_bridgedrive import Visualizer
...
class SensorAgent(BaseAgent, autonomous_agent.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file: str, _=None, __=None):
        ...
        # BridgeDrive - add step_num for forward_test in ddbm module
        self.training_config.step_num = self.config_closed_loop.step_num
    ...
    def run_step(self, input_data: dict, _, __=None) -> carla.VehicleControl:
        ...
        if (
            ...
            # BridgeDrive
            if self.step % self.config_closed_loop.viz_downsample_factor == 0:
                # Produce image
                image = Visualizer(
                    config=self.training_config,
                ...
                image = image[::2, ::2, :] # reduce image size
