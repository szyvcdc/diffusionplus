...
from lead.tfv6.planning_decoder_bridgedrive import PlanningDecoderDDBM # BridgeDrive
...
class TFv6(nn.Module):
    ...
    def __init__(
        ...
        if self.config.use_planning_decoder:
            if self.config.use_tfv5_planning_decoder:
                ...
                self.planning_decoder = PlanningDecoderDDBM(
                    input_bev_channels=self.backbone.num_lidar_features,
                    config=self.config,
                    device=self.device,
                ).to(self.device)
    ...
    def forward(self, data: dict[str, typing.Any]):
        self.log = {}
