...
# BridgeDrive
from lead.tfv6.diffusion_modules.model_diffusion_head_ddbm import TrajectoryHead
from lead.tfv6.diffusion_modules.multimodal_loss_ddbm import LossComputer
import numpy as np

logger = logging.getLogger(__name__)


class PlanningDecoderDDBM(nn.Module):
    @beartype
    def __init__(
        self, input_bev_channels: int, config: TrainingConfig, device: torch.device
    ):
        super().__init__()
        self.loss_computer_ddbm = LossComputer(config)
        self.device = device
        self.config = config
        ...
        self.trajectory_head = TrajectoryHead(
            num_poses=self.config.trajectory_sampling_num_poses,
            d_ffn=self.config.tf_d_ffn,
    ...
    def forward(
        ...
        if self.config.predict_spatial_path:
            ...
            if 'route' in data.keys():
                self.config.ddbm_training = True
                bs=data['route'].shape[0]

                if self.config.diffusion_speed:
                    target_speed_num = data['target_speed'].to(self.device, dtype=torch.float32) # Use DDBM to predict speed
                    tmp = torch.cat([target_speed_num.unsqueeze(1), torch.zeros((bs,1),device=self.device)], dim=1).unsqueeze(1)
                    gt_spd_checkpoints = torch.cat([data['route'].to(self.device), tmp], dim=1)
                    data['gt_spd_checkpoints'] = gt_spd_checkpoints
                    route = self.trajectory_head(ego_query=route_queries, bev_feature=bev_features, bev_spatial_shape=(64,64), gt_checkpoints=gt_spd_checkpoints)
                else:
                    gt_checkpoints = data['route'].to(self.device)
                    route = self.trajectory_head(ego_query=route_queries, bev_feature=bev_features, bev_spatial_shape=(64,64), gt_checkpoints=gt_checkpoints)

            else:
                self.config.ddbm_training = False
                route = self.trajectory_head(ego_query=route_queries, bev_feature=bev_features, bev_spatial_shape=(64,64))         
    ...
    def compute_loss(self, predictions, data: dict, loss: dict, log: dict):
        ...
        with torch.amp.autocast(device_type="cuda", enabled=False):
            if self.config.predict_ddbm:
                ddbm_route_label = data['route'].to(
                ...
                if self.config.diffusion_speed:
                    ...
                    tmp = torch.cat([target_speed_num.unsqueeze(1), torch.zeros((bs,1),device=self.device)], dim=1).unsqueeze(1)
                    gt_spd_checkpoints = torch.cat([data['route'].to(self.device), tmp], dim=1)
                    targets = {"trajectory": gt_spd_checkpoints}
                    bs=gt_spd_checkpoints.shape[0]
                else:
                    gt_checkpoints = data['route'].to(self.device)
                    reg_traj_loss = 0
                    cls_traj_loss = 0
                    targets = {"trajectory": gt_checkpoints}

                plan_anchor = self.plan_anchor.unsqueeze(0).repeat(bs,1,1,1)
                for idx, poses_cls in enumerate(predictions.pred_route['poses_cls_list']):
                    traj_cls_loss, traj_reg_loss = self.loss_computer_ddbm(None, poses_cls, targets, plan_anchor, predictions.pred_route['best_reg'])

                    cls_traj_loss += traj_cls_loss
                    reg_traj_loss += traj_reg_loss
                    
                loss["loss_ddbm_reg"] = reg_traj_loss
                loss["loss_ddbm_cls"] = cls_traj_loss
            ...
            # BridgeDrive: turn off predict_spatial_path
            # if self.config.predict_spatial_path:
                ...
                log.update(
                    {
                        "metric/route_ade": common_utils.average_displacement_error(
                            predictions.pred_route['best_reg'][...,:self.config.trajectory_sampling_num_poses,:], route_label # BridgeDrive config
                        ),
                        "metric/route_fde": common_utils.final_displacement_error(
                            predictions.pred_route['best_reg'][...,:self.config.trajectory_sampling_num_poses,:], route_label # SBridgeDrive config
                        ),
                    }
                )
