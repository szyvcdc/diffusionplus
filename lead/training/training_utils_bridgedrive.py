...
def initialize_model(
    ...
    # BridgeDrive
    from lead.tfv6.tfv6_bridgedrive import TFv6
    ...
    if config.load_file is not None:
        ...
        # BridgeDrive
        checkpoint = torch.load(config.load_file, map_location="cpu", weights_only=True)
        # checkpoint_state_dict = checkpoint['model_state_dict']

        if 'planning_decoder.query' in checkpoint:
            checkpoint_query_shape = checkpoint['planning_decoder.query'].shape
            model_query_shape = model.state_dict()['planning_decoder.query'].shape
            
            if checkpoint_query_shape != model_query_shape:
                print(f"Removing mismatched parameter: planning_decoder.query")
                print(f"Checkpoint shape: {checkpoint_query_shape}, Model shape: {model_query_shape}")
                del checkpoint['planning_decoder.query']
        msg = model.load_state_dict(checkpoint,strict=config.continue_failed_training)
        # msg = model.load_state_dict(
        #     torch.load(config.load_file, map_location="cpu", weights_only=True),
        #     strict=config.continue_failed_training,
        # )

        print(f"Loading checkpoint returns: {msg}")
